Quick wins (polish / watch-phone parity):
  ...
  - Add direction/bearing arrow to stop list rows (distance shown, but no pointer to tell user which way to walk)
  ...
  - Link to add favorite stop (help text or button explaining how to star a stop)
...
Medium features:
  ...
  - Phone haptic / notification for imminent arrivals (watch fires haptics at ≤2 min; phone has nothing)
  - Fix commute slot picker when no location data (picker candidates come from nearbyStops; empty if settings opens before location is available)
  - Configurable notification on alert (see also phone haptic item above)
